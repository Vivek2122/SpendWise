...
function EditModal({ onClose, transaction, queryKey }) {
	// transaction is an object: { _id, source, amount, date }

	const [formData, setFormData] = useState({
		source: transaction.source || "",
		amount: transaction.amount || "",
		date: transaction.date.slice(0, 10) || "", // for date input
	});

	const type = queryKey == "incomeTransactions" ? "income" : "expense";
		...
		<div className="fixed inset-0 z-50 bg-black bg-opacity-50 flex items-center justify-center px-4">
			<div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 animate-fade-in">
				...
				<div className="flex justify-between items-center mb-6">
					<h2 className="text-xl font-bold text-gray-800">Edit {type}</h2>
					<button
						onClick={onClose}
				...
				<form onSubmit={handleSubmit} className="space-y-5">
					...
					<button
    					...
    					${
  						...
  						`}
					>
						{editMutation.isLoading ? "Updating..." : "Update Income"}
					</button>
				</form>
			</div>
		</div>
